...
const SearchMapPage = ({
  ...
  const searchKakaoAPI = async ({
    ...
    let mySearchWord = historyPlaceName
      ? historyPlaceName
      : debouncePlaceInput
      ? debouncePlaceInput
      : placeInput;
    ...
    await axios
      .get(
        ...
        }&size=15&sort=accuracy&query=${encodeURI(mySearchWord)}`,
        {
          headers: {
      ...
      .then((res) => {
        ...
        switch (searchType) {
          ...
          case 'clickHistoryTypeSubmit':
            console.log('SUBMIT:: ,getPlaceData', getPlaceData);
            setPlaceDataToMap(getPlaceData);
            storeSearchPlace({
              type: 'submit',
